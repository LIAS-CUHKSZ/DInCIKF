function [X_next, costf_value,dX, K, H_time] = IMU_camera_inupdate(X_last,P_last, camera_m, dX_last, a_m, w_m, tc_last,SIGMA_SET, X_est, CAM_INTRISIC, IF_IEKF_update_bgba)
...
%     [X_time,P_time]=inEKF_propagate(X_last, P_last, w_m,  a_m, deltat,SIGMA_SET, IF_IEKF_update_bgba);
    ...
    for i=1:N_valid_camLm
        ...
        pfinC_time=(X_time.Rc)'*(p_f'-X_time.pc');
    ...
    X_next.Rc=X_next.Rimu*X_next.Rci;
    X_next.pc=(X_next.Rimu*X_next.pci')'+X_next.pimu;
    ...
    %% 4. Compute the Kalman gain
    % 
    J_inv=jaco_left_sum(delta_X);%eye(length(P_time));
    P_time_com = J_inv*P_time*J_inv';
    S=H_time*J_inv*P_time*J_inv'*H_time' + R;   %covariance of residual
    K=P_time*J_inv'*H_time'*inv(S);  %Kalman gain
    %% 5. Compute the correction $\Delta X$
